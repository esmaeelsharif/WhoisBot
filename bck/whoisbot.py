...
def check_domain_availability(whois_output: str) -> bool:
    ...
    for pattern in error_patterns:
        if re.search(pattern, whois_lower):
            ...
            logging.warning(f"WHOIS error detected: pattern '{pattern}' found in output")
            return False
    
    # Check for availability patterns
    for pattern in available_patterns:
        if re.search(pattern, whois_lower):
            logging.info(f"Domain appears available: pattern '{pattern}' found in output")
            return True
    
    # Check for definitive signs of registration
    for pattern in unavailable_patterns:
        if re.search(pattern, whois_lower):
            logging.info(f"Domain appears registered: pattern '{pattern}' found in output")
            return False
    
    # Check output length - very short outputs often mean "no match" in some WHOIS servers
    if len(whois_output.strip()) < 50:
        logging.info("Short WHOIS output, likely available domain")
        return True
    ...
    if len(lines) < 5:
        logging.info("Few output lines, likely available domain")
        return True
        
    # Default to unavailable if we can't determine for sure
    # This is the safest approach for domains we're uncertain about
    logging.info("Couldn't definitively determine domain status, defaulting to unavailable")
    return False
